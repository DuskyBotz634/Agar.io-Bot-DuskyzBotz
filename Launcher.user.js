// ==UserScript==
// @name        DuskyBotz634Launcher
// @namespace   DuskyBotz634Launcher
// @include     http://agar.io/
// @version     1.0
// @grant       none
// @author      youtube.com/user/DuskyPlaysMinecraft1
// ==/UserScript==
...
$.get('https://raw.githubusercontent.com/DuskyBotz634/Agar.io-bot/master/launcher.user.js?' + Math.floor((Math.random() * 1000000) + 1), function(data) {
    ...
    if (latestVersion > myVersion) {
        update("DuskyBotz634Launcher", "launcher.user.js", "https://github.com/DuskyBotz634/Agar.io-bot/blob/master/launcher.user.js/");
    }
...
})(window, document, 'script', '//www.google-analytics.com/analytics.js', 'DuskyBotz634');

DuskyBotz634('create', 'UA-62006573-2', 'auto');
DuskyBotz634('send', 'pageview');

window.ignoreStream = false,
    window.refreshTwitch = function() {
    $.ajax({
        url: "https://api.twitch.tv/kraken/streams/DuskyPlaysMinecraft",
        cache: false,
        dataType: "jsonp"
    }).done(function(data) {
        if (data["stream"] == null) {
            //console.log("theDuskyBotz634 is not online!");
            window.setMessage([]);
            window.onmouseup = function() {};
            window.ignoreStream = false;
        } else {
            //console.log("theDuskyBotz634 is online!");
            if (!window.ignoreStream) {
                window.setMessage(["twitch.tv/theDuskyBotz634 is online right now!", "Click the screen to open the stream!", "Press E to ignore."]);
                window.onmouseup = function() {
                    window.open("http://www.twitch.tv/DuskyPlaysMinecraft");
                };
            }
        }
    }).fail(function() {});
